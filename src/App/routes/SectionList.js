...
SectionRoute.post("/add-section",async (req, res) => {
    try {
...
SectionRoute.patch("/update-section",  async (req, res) => {
  try {
...
SectionRoute.delete("/delete-section",  async (req, res) => {
  try {
    const sectionId = req.body._id;
    // Ensure `_id` is a valid MongoDB ObjectId
    if (!isValidObjectId(sectionId)) {
...
SectionRoute.get("/search-section",  async (req, res) => {
  try {
...
SectionRoute.get("/section-data",  async (req, res) => {
  try {
    const GetSection = await SectionModel.findOne(req.body);
    res.send(GetSection);
  } catch (error) {
...
SectionRoute.get("/sections-data",  async (req, res) => {
  try {
