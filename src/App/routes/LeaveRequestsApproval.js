...
LeaveRequestRoute.post("/add-leave-approval",async (req, res) => {
    try {
...
LeaveRequestRoute.patch("/update-leave-approval-data",  async (req, res) => {
  try {
...
LeaveRequestRoute.delete("/delete-leave-approval",  async (req, res) => {
  try {
    const leaverequestId = req.body._id;
    // Ensure `_id` is a valid MongoDB ObjectId
    if (!isValidObjectId(leaverequestId)) {
...
LeaveRequestRoute.get("/search-leave-approval",  async (req, res) => {
  try {
