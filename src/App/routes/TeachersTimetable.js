...
TeachersTimetableRoute.post("/add-teacher-timetable",async (req, res) => {
    try {
...
TeachersTimetableRoute.patch("/update-teacher-timetable",  async (req, res) => {
  try {
    ...
    // Find the timetable by ID first
...
TeachersTimetableRoute.delete("/delete-teacher-timetable",  async (req, res) => {
  try {
    const timetableId = req.body._id;
    // Ensure `_id` is a valid MongoDB ObjectId
    if (!isValidObjectId(timetableId)) {
...
TeachersTimetableRoute.get("/search-teacher-timetable",  async (req, res) => {
  try {
...
TeachersTimetableRoute.get("/teacher-timetable-data",  async (req, res) => {
  try {
    const GetTimetable = await TimeTableModel.findOne(req.body);
    res.send(GetTimetable);
  } catch (error) {
...
TeachersTimetableRoute.get("/teachers-timetable-data",  async (req, res) => {
  try {
