const express = require("express");
const ComplaintsRoute = express.Router();
const {isValidObjectId} = require("../../utils/validation");
const ComplaintListModel = require("../../models/TeacherComplaints");
const { Error } = require("console");
const { AppuserAuth } = require("../../middlewares/auth");

ComplaintsRoute.post("/add-complaint",async (req, res) => {
    try {
      const Addcomplaint = new ComplaintListModel(req.body);
      await Addcomplaint.save();
      res.send("Added complaint Successfully");
    } catch (error) {
      res.status(400).send("Error adding the complaint");
    }
  }
);

ComplaintsRoute.patch("/update-complaint-data",  async (req, res) => {
  try {

    const complaintId = req.body._id;

    // Ensure _id is a valid MongoDB ObjectId
    if (!isValidObjectId(complaintId)) {
      return res.status(400).json({ error: "Invalid ID format" });
    }
    // Find the complaint by ID first
    let complaint = await ComplaintListModel.findById(complaintId);
    if (!complaint) {
      return res.status(404).json({ error: "complaint not found" });
    }
    // ✅ Update fields from request body
    Object.keys(req.body).forEach((key) => {
      complaint[key] = req.body[key];
    });
    // Save updated complaint
    await complaint.save();
    return res.json({
      message: "complaint data updated successfully",
      complaint,
    });
  } catch (error) {
    console.error("Error updating complaint:", error);
    return res.status(500).json({ error: "Something went wrong" });
  }
});

ComplaintsRoute.delete("/delete-complaint-data",  async (req, res) => {
  try {
    const complaintId = req.body._id;
    // Ensure _id is a valid MongoDB ObjectId
    if (!isValidObjectId(complaintId)) {
      return res.status(400).json({ error: "Invalid ID format" });
    }
    await ComplaintListModel.findByIdAndDelete(complaintId);
    res.send("complaint data deleted successfully");
  } catch (error) {
    res.status(400).send("Error deleting  the complaint data");
  }
});

ComplaintsRoute.get("/search-complaint-data",  async (req, res) => {
  try {
    const Getcomplaintdata = await ComplaintListModel.findOne(req.body);
    res.send(Getcomplaintdata);
  } catch (error) {
    res.status(400).send("complaint data not found");
  }
});

ComplaintsRoute.get("/complaint-data",  async (req, res) => {
  try {
    const Getcomplaintdata = await ComplaintListModel.findOne(req.body);
    res.send(Getcomplaintdata);
  } catch (error) {
    res.status(400).send("complaint data not found");
  }
});

ComplaintsRoute.get("/complaints-data",  async (req, res) => {
  try {
    const Getcomplaintdata = await ComplaintListModel.find();
    res.send(Getcomplaintdata);
  } catch (error) {
    res.status(400).send("complaint data not found");
  }
});

module.exports = ComplaintsRoute;