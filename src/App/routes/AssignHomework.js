...
const homeworksModel = require("../../models/AssignHomework");
...
homeworkRoute.post("/add-homework",upload.single("homeworkfile"),async (req, res) => {
    try {
        if (req.file) {
...
homeworkRoute.patch("/update-homework-data",upload.single("homeworkfile"),async (req, res) => {
    try {
      ...
      if (req.file && homework.homeworkfile) {
        const oldFilePath = path.join(__dirname, "..", "homework", homework.homeworkfile);
      ...
      // ✅ Update homework file if uploaded
      if (req.file) {
        homework.homeworkfile = req.file.filename; // Store the new file name
      }
