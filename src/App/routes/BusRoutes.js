...
BusRoute.post("/add-bus-route",async (req, res) => {
    try {
...
BusRoute.patch("/update-bus-route",  async (req, res) => {
  try {
...
BusRoute.delete("/delete-bus-route",  async (req, res) => {
  try {
    const busrouteId = req.body._id;
    // Ensure `_id` is a valid MongoDB ObjectId
    if (!isValidObjectId(busrouteId)) {
...
BusRoute.get("/search-bus-route",  async (req, res) => {
  try {
...
BusRoute.get("/bus-route-data",  async (req, res) => {
  try {
    const GetBusRoutedata = await BusRouteModel.findOne(req.body);
    res.send(GetBusRoutedata);
  } catch (error) {
...
BusRoute.get("/buses-routes-data",  async (req, res) => {
  try {
