...
FeesRoute.post("/add-fee",async (req, res) => {
    try {
...
FeesRoute.patch("/update-fee",  async (req, res) => {
  try {
...
FeesRoute.delete("/delete-fee",  async (req, res) => {
  try {
    const feesId = req.body._id;
    // Ensure `_id` is a valid MongoDB ObjectId
    if (!isValidObjectId(feesId)) {
...
FeesRoute.get("/search-fee",  async (req, res) => {
  try {
...
FeesRoute.get("/fee-data",  async (req, res) => {
  try {
    const GetFees = await FeesModel.findOne(req.body);
    res.send(GetFees);
  } catch (error) {
...
FeesRoute.get("/fees-data",  async (req, res) => {
  try {
