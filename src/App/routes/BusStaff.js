...
BusStaffRoute.post("/add-bus-staff",upload.fields([{ name: "profilePhoto" }, { name: "licensePhoto" }]),async (req, res) => {
    try {
...
BusStaffRoute.patch("/update-bus-staff",  upload.fields([{ name: "profilePhoto" }, { name: "licensePhoto" }]), async (req, res) => {
  try {
...
BusStaffRoute.delete("/delete-bus-staff",  async (req, res) => {
  try {
    const busstaffId = req.body._id;
    // Ensure `_id` is a valid MongoDB ObjectId
    if (!isValidObjectId(busstaffId)) {
...
BusStaffRoute.get("/search-bus-staff",  async (req, res) => {
  try {
...
BusStaffRoute.get("/bus-staff-data",  async (req, res) => {
  try {
    const GetBusStaffdata = await BusStaffModel.findOne(req.body);
    res.send(GetBusStaffdata);
  } catch (error) {
...
BusStaffRoute.get("/buses-staff-data",  async (req, res) => {
  try {
